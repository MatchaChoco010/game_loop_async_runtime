...
use crate::world::{Direction, GameState, World, HEIGHT, WIDTH};
...
async fn game_over(world: Arc<RwLock<World>>, w: Arc<Mutex<impl Write>>) {
    ...
    {
        let mut world = world.write().expect("Get world");
        world.should_stop_game = true;
    }
}

async fn game_clear(world: Arc<RwLock<World>>, w: Arc<Mutex<impl Write>>) {
    let size = terminal::size().unwrap();
    let offset_x = if size.0 / 2 < WIDTH + 2 {
        println!("Terminal space is too small!");
        0
...
async fn game_close(world: Arc<RwLock<World>>) {
    'update_loop: loop {
        {
            let world = world.read().expect("Get world");
...
async fn render(world: Arc<RwLock<World>>, w: Arc<Mutex<impl Write>>) {
    loop {
        let state = {
            let world = world.read().expect("Get world");
            let mut w = w.lock().expect("Get write");
        ...
        match state {
            ...
            GameState::GameClear => {
                game_clear(Arc::clone(&world), Arc::clone(&w)).await;
                break;
            }
            GameState::GameOver => {
                game_over(Arc::clone(&world), Arc::clone(&w)).await;
                break;
            }
        }
    }
...
pub async fn render_system(world: Arc<RwLock<World>>, w: Arc<Mutex<impl Write>>) {
    {
        let mut w = w.lock().expect("Get write");
        execute!(w, EnterAlternateScreen).unwrap();
    }

    let render = render(Arc::clone(&world), Arc::clone(&w)).fuse();
    let close = game_close(Arc::clone(&world)).fuse();
    pin_mut!(render);
    pin_mut!(close);
    select! {
        _ = render => (),
